...
export function PlanLabelTypeForm({
  ...
  const [quantity, labelSize, extraRolls, materialCode, assignedRolls] = useWatch({
    control,
    name: ["quantity", "labelSize", "extraRolls", "materialCode", "assignedRolls"],
  });

  const material = materialByCode.get(materialCode || "");
  const required = calculateRequiredRolls(Number(quantity) || 0, Number(labelSize) || 0);
  const totalRolls = calculateTotalRolls(required, Number(extraRolls) || 0);
  const totalWeight = calculateTotalWeight(material?.weightPerRoll ?? 0, totalRolls);
  const assigned = Number(assignedRolls) || 0;
  ...
  const available = balance !== undefined ? balance + releasing : undefined;
  const insufficient = available !== undefined && assigned > available;
  ...
  return (
    ...
    >
      {/* A real <form> so Enter submits and the browser runs its own validation pass. */}
      <form onSubmit={submit} className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <AppInput
            ...
            step="any"
            disabled={busy}
            error={errors.extraRolls?.message}
            {...register("extraRolls", { valueAsNumber: true })}
          />
        </div>
        ...
        {/* Read-only, derived from the material code — shown so the planner can confirm
            the right material before stock is committed to it. */}
        {material && (
          <div className="flex flex-wrap items-center gap-x-6 gap-y-1 rounded-md bg-muted/30 px-3 py-2 text-xs">
            ...
            <Stat
              ...
              value={available !== undefined ? `${formatNumber(available, 0)} rolls` : "No ledger"}
            />
            <Stat label="Assigned Weight (kg)" value={formatNumber(material.weightPerRoll * assigned, 4)} />
          </div>
        )}
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-1 border-t border-border/50 pt-3 text-xs">
          ...
          <Stat label="Total Rolls Issued" value={formatNumber(totalRolls, 0)} emphasis />
          <Stat label="Total Weight (kg)" value={formatNumber(totalWeight, 4)} />
        </div>

        {insufficient && (
          <Warning>
            Inventory has {formatNumber(available ?? 0, 0)} rolls of {materialCode} — cannot assign{" "}
            {formatNumber(assigned, 0)}.
          </Warning>
        )}
        {overAllocated && (
          <Warning>
